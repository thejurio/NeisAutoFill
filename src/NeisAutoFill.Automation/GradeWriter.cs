...
/// <summary>
/// 등급을 나이스 교과별 평가 그리드의 등급 콤보에 입력한다 (NarrativeWriter 와 대칭, R11).
/// 안전정책: dry-run, 매칭 검증(resolveMatch), 멱등성, 입력 후 재검증+1회 재시도, 저장 미자동화.
/// 화면 판별·이동·조회·저장은 NeisEngine(세션) 담당 — 여기는 조회된 그리드에 쓰는 일만.
/// </summary>
public sealed class GradeWriter(IPage page, GridScroller scroller, ComboBoxDriver combo)
{
    /// <param name="grid">조회로 이미 떠 있는 평가 그리드 (엔진이 찾아 넘긴다).</param>
    /// <param name="screenSubject">화면 과목 (매칭 확인 창에 표시).</param>
    public async Task<RunReport> RunAsync(
        ...
        Func<MatchContext, Task<MatchDecision?>>? resolveMatch,
        CancellationToken ct)
    {
        ...
        {
            decision = await resolveMatch(new MatchContext(screenSubject, sheet.SubjectName, rowMap, missing));
            if (decision is null)
            {
                Log("사용자가 입력을 취소했습니다.");
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            {
                var (ok, why) = await SetGradeAsync(grid, t, ct);
                if (ok)
                {
                    done.Add(t);
    ...
    private async Task<(bool ok, string why)> SetGradeAsync(
        ILocator grid, GradeTask task, CancellationToken ct)
    {
        ...
        {
            var cell = await EnsureRowVisibleAsync(grid, task.RowIndex, ct);
            if (cell is null) return (false, "행을 화면에 못 띄움");

            try
            {
                var cur = await ComboBoxDriver.ReadValueAsync(cell);
                if (cur == neisText) return (true, "이미 설정됨");   // 멱등성

                var pick = await combo.OpenAndPickAsync(cell, neisText);
                if (pick.Ok)
                {
                    // ★ 재검증: 콤보를 fresh 로 다시 읽어 목표값 확인
                    var cell2 = await GetFreshComboAsync(grid, task.RowIndex);
                    var val = cell2 is null ? "" : await ComboBoxDriver.ReadValueAsync(cell2);
    ...
    {
        var cell = await GetFreshComboAsync(grid, idx);
        if (cell is null)
        {
